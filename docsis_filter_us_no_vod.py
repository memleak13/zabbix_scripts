#!/usr/bin/python3
"""
    Name:       docsis_filter_us_no_vod.py
    Version:    0.1
    Author:     JM, 25.07.14
    
    Adjusted Version to exclude all vod interfaces (as SNR is 0 and will trigger
    alarm otherwise)
...
def filter_table(snmp_table):
    ...
    for row in snmp_table:
        ...
        #ifAdminState: 1 (Up)
        if (row[2] == filter_on_ifType and row[3] == filter_on_ifAdminState and
            "VOD" not in row[1]):
